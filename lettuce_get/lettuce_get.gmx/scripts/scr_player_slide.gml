...
{
    if sign(image_xscale) == sign(x_vel) x_vel *= .995;
    if sign(image_xscale) != sign(x_vel) x_vel *= .95;
...
if ((abs(x_vel) < .75 && place_meeting(x,y+1,obj_wall))) || (place_meeting(x+sign(image_xscale),y,obj_ground) && (down)) || (place_meeting(x-sign(image_xscale),y,obj_ground) && (down))
{
